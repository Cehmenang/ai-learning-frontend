...
function DocumentCard({ doc }: { doc: any}) {
  const colors = typeColors[doc.type] || typeColors.TXT;
  return (
    <div
    ...
    >
      <div className="p-4 pb-3 flex-1">
        <div className="flex items-start justify-between mb-3">
          <div
            className={`flex items-center gap-1.5 px-2 py-0.5 rounded-md ${colors.bg}`}
          >
            <div className={`w-1.5 h-1.5 rounded-full ${colors.dot}`} />
            <span className={`${colors.text}`} style={{ fontSize: "11px", fontWeight: 600 }}>
              {doc.title.endsWith('.pdf') && 'PDF'}
            </span>
          </div>
          <ArrowUpRight className="w-4 h-4 text-[#3a3a5a] group-hover:text-[#6366f1] transition-colors" />
        </div>
        <h3 className="text-white mb-2 line-clamp-2 group-hover:text-[#818cf8] transition-colors" style={{ fontSize: "14px", fontWeight: 500 }}>
          {doc.title}
        </h3>
        <div className="text-slate-500 flex gap-x-3 items-center" style={{ fontSize: "12px", lineHeight: "1.6" }}>
          <Link href={`document/${doc.id}`} className="border-1 border-slate-700 px-4 py-[2px] rounded-xs hover:text-slate-300 hover:border-slate-500 transition">Watch</Link>
          <Link href={`document/${doc.id}/quizzes`} className="border-1 border-slate-700 px-4 py-[2px] rounded-xs hover:text-slate-300 hover:border-slate-500 transition">Quizzes</Link>
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-t border-[#1e1e2e] flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 text-[#3a3a5a]">
          ...
          <span style={{ fontSize: "11px" }}>{new Date(doc.createdAt).toLocaleDateString('id-ID')}</span>
        </div>
      </div>
    </div>
  );
...
function DocumentRow({ doc }: { doc: any}) {
  ...
  return (
    <Link
      ...
      className="group flex items-center gap-4 p-4 rounded-xl border border-[#1e1e2e] hover:border-[#6366f1]/30 hover:bg-[#0e0e18] transition-all duration-200"
    >
      <div
      ...
      <div className="flex-1 min-w-0">
        <p className="text-white truncate group-hover:text-[#818cf8] transition-colors" style={{ fontSize: "14px", fontWeight: 500 }}>
          {doc.title}
        </p>
      ...
      <div
        className={`hidden sm:flex items-center gap-1 px-2 py-0.5 rounded-md ${colors.bg} flex-shrink-0`}
      >
        <span className={`${colors.text}`} style={{ fontSize: "11px", fontWeight: 600 }}>
          {doc.type}
        </span>
...
export default function DocumentsPage() {
  ...
  const filtered = documents.filter((doc) => {
    ...
    const matchFilter = filter === "all" || doc.title.includes(`${types}`) === filter;
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link href="/upload" className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#6366f1] hover:bg-[#5558e8] text-white transition-all duration-200 shadow-lg shadow-indigo-500/20 self-start sm:self-auto">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-0.5">
          {types.map((type) => (
            <button
              ...
              className={`px-3 py-2 rounded-xl whitespace-nowrap transition-all duration-200 flex-shrink-0 ${
                filter === type
                  ? "bg-[#6366f1]/15 text-[#818cf8] border border-[#6366f1]/30"
                  : "bg-[#0e0e18] text-[#4a4a6a] border border-[#1e1e2e] hover:text-[#8a8aaa] hover:border-[#2e2e4e]"
              }`}
              style={{ fontSize: "13px" }}
            >
              {type === "all" ? "Semua" : type}
      ...
      {search && (
        <p className="text-[#4a4a6a]" style={{ fontSize: "13px" }}>
          Menampilkan{" "}
          <span className="text-white">{filtered.length}</span> hasil untuk "
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-16 h-16 rounded-2xl bg-[#0e0e18] border border-[#1e1e2e] flex items-center justify-center mb-4">
            <FileText className="w-8 h-8 text-[#3a3a5a]" />
          </div>
